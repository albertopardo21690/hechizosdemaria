    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid @if($hasImg) lg:grid-cols-2 @endif gap-12 items-center">
            <div class="space-y-5 {{ $align === 'center' ? 'text-center mx-auto max-w-2xl' : '' }}">
                @if($p['eyebrow'] ?? null)<span class="text-pink-500 text-xs tracking-[0.3em] uppercase block">{{ $p['eyebrow'] }}</span>@endif
                <h1 class="font-heading text-4xl md:text-6xl text-pink-700 leading-tight">{{ $p['heading'] ?? '' }}</h1>
                @if($p['subheading'] ?? null)<p class="font-heading text-lg md:text-xl italic text-pink-600">{{ $p['subheading'] }}</p>@endif
                @if($p['body'] ?? null)<p class="text-gray-700 leading-relaxed max-w-lg {{ $align === 'center' ? 'mx-auto' : '' }}">{{ $p['body'] }}</p>@endif
                @if($p['cta_text'] ?? null)<div class="pt-3"><a href="{{ $p['cta_url'] ?? '#' }}" class="btn-mystic">{{ $p['cta_text'] }}</a></div>@endif
            </div>
            @if($hasImg)
                <div class="relative">
