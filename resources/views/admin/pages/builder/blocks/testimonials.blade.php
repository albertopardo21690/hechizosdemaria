@if($editing)
    <div class="space-y-3 text-sm">
        <label class="block"><span class="block text-xs uppercase tracking-widest text-gray-600 mb-1">Título de la sección</span><input type="text" wire:model.lazy="{{ $path }}.props.title" class="w-full border border-gray-300 rounded-md px-3 py-2"></label>
        <label class="flex items-center gap-2"><input type="checkbox" wire:model.lazy="{{ $path }}.props.only_featured" class="accent-pink-500"> Mostrar solo destacados y aprobados</label>
                ...
                <div class="max-h-64 overflow-y-auto space-y-1 border border-pink-200 rounded-md p-2">
                    @forelse($testimonialsList as $t)
                        <label class="flex gap-2 items-start py-1 hover:bg-pink-50 px-2 rounded">
                            <input type="checkbox" wire:model.lazy="{{ $path }}.props.ids" value="{{ $t->id }}" class="accent-pink-500 mt-1">
